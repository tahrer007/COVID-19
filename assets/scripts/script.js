...
//------------------------- getCountryCovidData -------------------
const getCountryCovidData = async (CountryCode,isCountrySellected,continent) => {
  try {
    ...
    if(!isCountrySellected) calculateContinentCovidData(countryData.data.latest_data,continent)
    if (!response.ok) {
...
//getCountryCovidData("SA",false,'Asia')
